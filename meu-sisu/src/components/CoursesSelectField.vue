  ...
  <div class="container">
    <div class="select-btn" :class="expanded ? 'open' : ''">
      <input @focusin="expanded = true" @focusout="close" class="btn-text" type="text" v-model="searching"
        :placeholder="selected ? selected.curso : placeholder" />
    ...
    <ul class="list-items" v-if="filteredCourses.length > 0">
      <li
        class="item" v-for="course in filteredCourses" @click="select(course as CourseSearch)"
        :class="selected === course ? 'checked' : ''">
        <span class="item-text">{{ (course as CourseSearch).curso }}</span>
      </li>
    </ul>
...
<script lang="ts">
import type { Course, CourseSearch } from "@/models";
import { placeholder } from "@babel/types";
import { defineComponent } from "vue";

export default defineComponent({
  name: "courseSelectField",
  ...
  computed: {
    filteredCourses() {
      const filtered = this.courses.filter((course: any) => {
        return this.removeAccents(course.curso as string)
          .toLocaleLowerCase()
          .includes(this.removeAccents(this.searching.toLowerCase()));
      });

      if (filtered.length === 1 && this.selected) return this.courses;
  ...
  methods: {
    ...
    select(location: CourseSearch) {
      this.selected = location;
      this.searching = location.curso;
