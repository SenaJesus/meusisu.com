  ...
  <div class="container">
    <div class="select-btn" :class="expanded ? 'open' : ''">
      <input @focusin="expanded = true" @focusout="close" class="btn-text" type="text" v-model="searching"
        :placeholder="selected ? selected.estado : placeholder"/>
    ...
    <ul class="list-items" v-if="filteredLocations.length > 0">
      <li class="item" v-for="location in filteredLocations" @click="select(location as State)"
        :class="selected === location ? 'checked' : ''">
        <span class="item-text">{{ (location as State).estado }}</span>
      </li>
    </ul>
...
export default defineComponent({
  ...
  computed: {
    filteredLocations() {
      ...
      const filtered = this.locations.filter((location: any) => {
        return this.removeAccents((location.estado as string)
          .toLocaleLowerCase())
          .startsWith(this.removeAccents(this.searching.trim().toLowerCase()));
      });

      if (filtered.length === 1 && this.selected) return this.locations;
  ...
  methods: {
    ...
    select(institution: State) {
      ...
      this.searching = institution.estado;
  ...
  watch: {
    searching(newValue, oldValue) {
      if (newValue.trim() === "" && this.selected !== null)
